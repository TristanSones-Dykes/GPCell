...
$$\
The oscillatory parameter $\beta$ must include 0 in its support, so a half-normal distribution was chosen. $X \sim \text{HalfNormal}(\sigma^2)$, with the condition that 95% of its mass lies below the upper bound of its original underlying prior:
...
a2, b2 = 0.1, 4.0
p_upper = 0.95
sigma2 = b2 / (np.sqrt(2) * erfinv(p_upper))
x2 = np.linspace(0, b2 * 1.2, 1000)
halfnorm_pdf = np.sqrt(2 / np.pi) / sigma2 * np.exp(-x2**2 / (2 * sigma2**2))
