...
In our case, we will be training models with periodic and aperiodic kernels, then assessing and using the quality of their fits to determine whether or not the underlying gene expression is oscillating or not.
...
GPCell is structured into clearly defined, modular components. A schematic class diagram of the main components – `OscillatorDetector`, `GaussianProcess`, `GPRConstructor`, and supporting utility modules – is provided in Figure X

`GaussianProcess`**:** A wrapper around GPflow's Gaussian Process models. It encapsulates the logic for optimising the given model type, predicting on new data, and extracting hyperparameters and values from the kernel. It is instantiated with a `GPRConstructor`, which defines the inference type (i.e: MLE vs MCMC), kernel, priors, and trainable hyperparameters.
...
Additionally, `Joblib`'s backend `loky` can pickle or serialise more complex objects as well as those defined in notebooks, allowing for a more a more user-friendly interface and efficient encapsulation.
...
**Just-In-Time Compilation:** For the computationally intensive Gillespie simulations, GPCell utilises `Numba`, a Just-In-Time compiler for Python. This lets you convert Python code (exclusively written in base Python and `Numpy`) into optimised machine code at runtime.

\newpage

### Extensibility and Modularity

GPCell’s design strongly emphasises extensibility, enabling researchers to customise various components to their specific requirements.\
This is enabled by a strong type system integrated throughout the library and the large TensorFlow (Probability) backend.

**Custom Kernels:** GPCell supports the creation and combination of user-defined kernels, facilitating nuanced modelling of biological systems. Users can readily implement new kernels or combine existing ones using arithmetic operations, enhancing model flexibility.
...
To create a new kernel, all you have to do is inherit from `Kernel` – or one of its subclasses like `IsotropicStationary`; you could also take any class/TensorFlow `Module` and make it compatible with the `Kernel` interface.\
...
This uses GPflow's `IsotropicStationary` kernel base class, this is for stationary kernels that only depend on $r = || \mathbf x^\prime - \mathbf x ||_2$ requiring you to implement $k(r)$ or $k(r^2)$, $k(r) = k(\mathbf x^\prime, \mathbf x)$.

**Prior Distributions:** Through the `GPRConstructor`, researchers can easily define and adjust priors for GP hyperparameters, allowing more precise control over model behaviour and incorporating domain-specific knowledge effectively. This is shown in the typing of `GPPrior` and `GPPriorFactory` in `types.py`:
