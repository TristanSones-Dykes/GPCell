...
def distributed_fit_models(
    ...
    """
    Fit GP models to each cell using TensorFlow's tf.distribute.MirroredStrategy.

    Within the MirroredStrategy scope, we create and fit replicates for each cell.
    This allows TensorFlow to distribute computations across available devices (GPUs/CPUs)
    without the overhead of copying data between processes.
    ...
    if preprocess == 0:
        ...
        Y_processed = [(y - np.mean(y)) for y in Y]
    elif preprocess == 2:
        Y_processed = [(y - np.mean(y)) / np.std(y) for y in Y]
    else:
        Y_processed = Y

    # Create a distribution strategy.
    strategy = tf.distribute.MirroredStrategy()
    results: List[List[GaussianProcess]] = []

    with strategy.scope():
        # For each cell, fit the replicates inside the strategy's scope.
        for i, (x, y, constructor) in enumerate(zip(X, Y_processed, constructors)):
            cell_models = []
            for rep in range(replicates):
                gp_model = GaussianProcess(constructor)
                # Fit the model (runs in eager mode under the strategy scope).
                gp_model.fit(x, y, Y_var, verbose)
                cell_models.append(gp_model)
            results.append(cell_models)
    return results
